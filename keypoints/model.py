...
from metrics import DetectedKeypoint, Keypoint, KeypointAPMetrics
from heatmap import (
    BCE_loss,
...
class KeypointDetector(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(maximal_gt_keypoint_pixel_distances, str):
            maximal_gt_keypoint_pixel_distances = [int(val) for val in maximal_gt_keypoint_pixel_distances.strip().split(" ")]
        ...
        self.ap_test_metrics = [KeypointAPMetrics(self.maximal_gt_keypoint_pixel_distances) for _ in self.keypoint_channel_configuration]
    ...
    def shared_step(self, batch, batch_idx, include_visualization_data_in_result_dict=False) -> Dict[str, Any]:
        ...
        img_shape = input_images[0].shape[1:]
        heatmap_shape = (135, 240)
        gt_heatmaps = [
            create_heatmap_batch(
                img_shape,
                heatmap_shape,
        ...
        for channel_idx in range(len(self.keypoint_channel_configuration)):
            ...
            with torch.no_grad():
                channel_gt_losses.append(BCE_loss(gt_heatmaps[channel_idx], gt_heatmaps[channel_idx]))  # For reference, the loss of the GT heatmap with itself.

            # pass losses and other info to result dict
            result_dict.update({f"{self.keypoint_channel_configuration[channel_idx]}_loss": channel_losses[channel_idx].detach()})
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        result_dict = self.shared_step(
            train_batch,
            batch_idx,
            include_visualization_data_in_result_dict=include_vis_data,
        )
    ...
    def visualize_predictions_channels(self, result_dict):
        ...
        # return image_grids
        stacked = torch.stack(gt_heatmaps)
        combined_gt, _ = torch.max(stacked, dim=0)

        combined_preds, _ = torch.max(predicted_heatmaps, dim=1)
        grid = visualize_predicted_heatmaps(input_images.cpu(), combined_preds.cpu(), combined_gt.cpu())
        return [grid]
    ...
    def log_channel_predictions_grids(self, image_grids, mode: str):
        for channel_configuration, grid in zip(self.keypoint_channel_configuration, image_grids):
            ...
            self.logger.experiment.log({"image": wandb.Image(grid, caption=image_caption, file_type="jpg")})
    ...
    def update_channel_ap_metrics(
        ...
        formatted_gt_keypoints = [[Keypoint(int(k[0]), int(k[1])) for k in frame_gt_keypoints] for frame_gt_keypoints in gt_keypoints]
        batch_detected_channel_keypoints = self.extract_detected_keypoints_from_heatmap(predicted_heatmaps.unsqueeze(1))
    ...
    def compute_and_log_metrics_for_channel(self, metrics: KeypointAPMetrics, channel: str, training_mode: str) -> List[float]:
        """
    ...
    def extract_detected_keypoints_from_heatmap(self, heatmap: torch.Tensor) -> List[DetectedKeypoint]:
        ...
        Args:
        heatmap (torch.Tensor) : 1 x H x W tensor that represents a heatmap.
        """
        ...
        keypoints, scores = get_keypoints_from_heatmap_batch_maxpool(  # Needz batch dim
            heatmap_to_extract_from,
            self.max_keypoints,
            self.minimal_keypoint_pixel_distance,
            return_scores=True,
        )
        detected_keypoints = [[[] for _ in range(heatmap_to_extract_from.shape[1])] for _ in range(heatmap_to_extract_from.shape[0])]
