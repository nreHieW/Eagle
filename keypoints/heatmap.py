...
def BCE_loss(input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    """
    return -(target * torch.clip(torch.log(input + 1e-10), -100, 100) + (1 - target) * torch.clip(torch.log(1 - input + 1e-10), -100, 100)).mean()
...
def create_heatmap_batch(
    ...
    batch_heatmaps = [generate_channel_heatmap(img_shape, heatmap_shape, keypoints[i], sigma, device) for i in range(len(keypoints))]
...
def generate_channel_heatmap(
    ...
    Args:
        image_size: Tuple(int,int) that specify (H,W) of the original image
        heatmap_size: Tuple(int,int) that specify (H,W) of the heatmap
    ...
    if keypoints.numel() == 0:
        ...
        return torch.zeros(heatmap_size, device=device)
...
def get_keypoints_from_heatmap_scipy(heatmap: torch.Tensor, min_keypoint_pixel_distance: int, max_keypoints: int = 20) -> List[Tuple[int, int]]:
    """
...
def get_keypoints_from_heatmap_batch_maxpool(
    heatmap: torch.Tensor, max_keypoints: int = 20, min_keypoint_pixel_distance: int = 1, abs_max_threshold: Optional[float] = None, rel_max_threshold: Optional[float] = None, return_scores: bool = False, scale: int = 4
        ...
        for channel_idx in range(n_channels):
            ...
            for candidate_idx in range(candidates.shape[0]):
                ...
                if scores[batch_idx, channel_idx, candidate_idx] > threshold:
                    # convert to (u,v)
                    filtered_indices[batch_idx][channel_idx].append((candidates[candidate_idx][::-1] * scale).astype(int))
